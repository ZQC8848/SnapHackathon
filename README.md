...
| Component | Description |
|-----------|-------------|
| **Line.ts** | Dynamic line renderer between two SceneObjects, updated every frame |
| **ClosedPolyline.ts** | Closed polyline connecting an array of SceneObjects |
| **Spline.ts** | Smooth Catmull-Rom spline through control point SceneObjects |
| **Spiral.ts** | Configurable spiral (linear, exponential, logarithmic, helix) centered on a SceneObject |
| **Circle.ts** | Circle gizmo rendered around a center SceneObject |
...
| **Logging (TypeScript)** | All TypeScript scripts use `Logger` from `Utilities.lspkg` instead of raw `print()` calls<br>• `enableLogging` inspector toggle for general logs<br>• `enableLoggingLifecycle` toggle for lifecycle event traces<br>• `logger.debug()`, `logger.info()`, `logger.warn()`, `logger.error()` |
| **Event Binding (TypeScript)** | All TypeScript scripts use `@bindUpdateEvent` and `@bindStartEvent` from `SnapDecorators.lspkg`<br>• Replaces manual `createEvent("UpdateEvent").bind(...)` boilerplate in `onAwake()`<br>• Reduces risk of missed event cleanup and simplifies lifecycle |
| **Inspector UI** | All TypeScript scripts include standardized inspector styling<br>• `@ui.label(...)` with colored HTML header and description per component<br>• `@ui.separator` between logical input groups<br>• All `@input` properties annotated with `@hint(...)` for in-editor documentation<br>• Logging section always placed at the bottom of inspector inputs |
